...
function BlogPost(props: BlogPostProps) {
  ...
  // Function to replace '||' with <br> tag
  const formatListItems = (text: string) => {
    ...
    return null; // Return null if text is undefined or empty
  };
  ...
  return (
    <div className="col-md-20 mb-4">
      ...
      <section>
        ...
        <img alt="" src={images[0]} className="img-fluid" />
        <div style={{ margin: "30px" }}>
          {/* Replace '||' with <br> tag in listItems[1] */}
          <p>{formatListItems(listItems[0])}</p>
          <p>{formatListItems(listItems[1])}</p>
          <p>{formatListItems(listItems[2])}</p>
          <p>{formatListItems(listItems[3])}</p>
          <p>{formatListItems(listItems[4])}</p>
        </div>
      </section>
